...
export enum REPORT_TEMPLATE {
  /** 摘要 */
  SUMMARY_TEXT = `
  脉络洞察安全服务团队于{{start_date}}至{{end_date}}，对{{target_system}}进行了全面的渗透测试。测试手段主要通过模拟黑客攻击手法对{{target_system}}开展测试并发现安全隐患；本次安全测试共发现了{{risk_total}}个安全漏洞，按漏洞风险分布为：超危风险{{risk_high}}个，高风险{{risk_medium}}个，中风险{{risk_low}}个，低风险{{risk_info}}个。
  脉络洞察安全安全服务团队发现存在的安全漏洞主要为：
  1) 请求头安全策略
  本次安全测试综合风险评级：低风险
  从本次渗透测试结果来看，{{target_system}}在安全防护方面达到优秀级别。
  建议{{target_system}}从以下三个方面进行安全整改，进一步提高网络安全防护和管理水平：
